...
public class MoodleUserProviderFactory implements ManagedServiceFactory {
   ...
   * The logger
   */
  protected static final Logger logger = LoggerFactory.getLogger(MoodleUserProviderFactory.class);
  ...
  /**
   * The key to look up the list of Instructor roles on the Moodle instance
   */
  private static final String MOODLE_INSTRUCTOR_ROLES_KEY = "org.opencastproject.userdirectory.moodle.instructor.roles";
  ...
  public void updated(String pid, Dictionary properties) throws ConfigurationException {
    ...
    String url = (String) properties.get(MOODLE_URL_KEY);
    if (StringUtils.isBlank(url)) throw new ConfigurationException(MOODLE_URL_KEY, "is not set");

    String token = (String) properties.get(MOODLE_TOKEN_KEY);
    if (StringUtils.isBlank(url)) throw new ConfigurationException(MOODLE_TOKEN_KEY, "is not set");
    ...
    String instructorRoleList = (String) properties.get(MOODLE_INSTRUCTOR_ROLES_KEY);
    ...
    if (!StringUtils.isEmpty(instructorRoleList)) {
      ...
      logger.info("Moodle instructor roles: {}", Arrays.toString(roles));
    } else {
      // Default instructor roles
      // TODO: check if these are the correct roles for Moodle
    ...
    MoodleUserProviderInstance provider = new MoodleUserProviderInstance(
        pid, org, url, token, coursePattern, userPattern, instructorRoles, cacheSize, cacheExpiration
    );
